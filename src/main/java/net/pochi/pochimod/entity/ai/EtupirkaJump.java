...
public class EtupirkaJump extends JumpGoal {
    private static final int[] STEPS_TO_CHECK = new int[]{0, 1, 4, 5, 6, 7};
    private final Etupirka dolphin;
    private final int interval;
    private boolean breached;

    public EtupirkaJump(Etupirka p_25168_, int p_25169_) {
    ...
    public void start() {
        Direction direction = this.dolphin.getMotionDirection();
        this.dolphin.setDeltaMovement(this.dolphin.getDeltaMovement().add((double)direction.getStepX() * 0.6D, 1.2D, (double)direction.getStepZ() * 0.6D));
    }
