...
public class DoeEntity extends AbstractHorse {
    ...
    public DoeEntity(EntityType<? extends DoeEntity> entityType, Level level) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new BreedGoal(this, 1.0D, DeerEntity.class));
    ...
    public class DoeStrippedGoal extends MoveToBlockGoal {
        ...
        public DoeStrippedGoal(double pSpeedModifier, int pSearchRange, int pVerticalSearchRange) {
            super(DoeEntity.this, pSpeedModifier, pSearchRange, pVerticalSearchRange);
        }
        ...
        public void tick() {
            ...
            } else if (!this.isReachedTarget() && DoeEntity.this.random.nextFloat() < 0.05F) {
                DoeEntity.this.playSound(SoundEvents.HORSE_STEP, 1.0F, 1.0F);
            }
        ...
        private void stripped(BlockState pState , BlockPos pos) {
            int j = 1 + DoeEntity.this.level().random.nextInt(2);
            ItemStack itemstack = DoeEntity.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty()) {
                DoeEntity.this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(ModItems.CINNAMON.get()));
                --j;
            }

            if (j > 0) {
                Block.popResource(DoeEntity.this.level(), this.blockPos, new ItemStack(ModItems.CINNAMON.get(), j));
            }

            DoeEntity.this.playSound(SoundEvents.AXE_STRIP, 1.0F, 1.0F);
            //DoeEntity.this.level().setBlock(pos, ModBlocks.STRIPPED_CINNAMON_LOG.get().defaultBlockState(),11);
        }

        public boolean canUse() {
            return !DoeEntity.this.isSleeping() && super.canUse();
        }
