...
public class Folivore extends Animal implements FlyingAnimal {
    ...
    public Folivore(EntityType<? extends Animal> entityType, Level level) {
    ...
    public class DoeStrippedGoal extends MoveToBlockGoal {
        ...
        public DoeStrippedGoal(double pSpeedModifier, int pSearchRange, int pVerticalSearchRange) {
            super(Folivore.this, pSpeedModifier, pSearchRange, pVerticalSearchRange);
        }
        ...
        protected void onReachedTarget() {
            if (net.neoforged.neoforge.common.NeoForge.EVENT_BUS.post(new net.neoforged.neoforge.event.entity.EntityMobGriefingEvent((net.minecraft.server.level.ServerLevel)Folivore.this.level(), Folivore.this)).canGrief()) {
                BlockState blockstate = Folivore.this.level().getBlockState(this.blockPos);
        ...
        private void stripped(BlockState pState , BlockPos pos) {
            int j = 1 + Folivore.this.level().random.nextInt(2);
            ItemStack itemstack = Folivore.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty()) {
                Folivore.this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(Items.BONE_MEAL));
                --j;
            }

            if (j > 0) {
                Block.popResource(Folivore.this.level(), this.blockPos, new ItemStack(Items.BONE_MEAL, j*8));
            }

            Folivore.this.playSound(SoundEvents.SHEEP_SHEAR, 1.0F, 1.0F);
            Folivore.this.level().setBlock(pos, Blocks.AIR.defaultBlockState(),11);
        }

        public boolean canUse() {
            return !Folivore.this.isSleeping() && super.canUse();
        }
