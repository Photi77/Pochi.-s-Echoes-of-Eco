...
public class Butterfly extends Animal implements GeoEntity{
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(3, new ButterflyHervGoal(1.0F,12,4));
    }
    ...
    public class ButterflyHervGoal extends MoveToBlockGoal {
        private static final int WAIT_TICKS = 40;
        protected int ticksWaited;

        public ButterflyHervGoal(double pSpeedModifier, int pSearchRange, int pVerticalSearchRange) {
            super(Butterfly.this, pSpeedModifier, pSearchRange, pVerticalSearchRange);
        }
        ...
        protected boolean isValidTarget(LevelReader pLevel, BlockPos pPos) {
            ...
            return blockstate.is(ModBlocks.QUEEN_OF_NIGHT.get());
        }
        ...
        public void tick() {
            ...
            } else if (!this.isReachedTarget() && Butterfly.this.random.nextFloat() < 0.5F) {
                Butterfly.this.playSound(SoundEvents.HOE_TILL, 1.0F, 1.0F);
            }
        ...
        private void stripped(BlockState pState , BlockPos pos) {
            int j = 1 + Butterfly.this.level().random.nextInt(2);
            ItemStack itemstack = Butterfly.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty()) {
                Butterfly.this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(ModBlocks.QUEEN_OF_NIGHT.get()));
                --j;
            }

            if (j > 0) {
                Block.popResource(Butterfly.this.level(), this.blockPos, new ItemStack(ModBlocks.QUEEN_OF_NIGHT.get(), j));
            }

            Butterfly.this.playSound(SoundEvents.FOX_SNIFF, 50.0F, 50.0F);
            Butterfly.this.level().setBlock(pos, Blocks.AIR.defaultBlockState(),11);
        }

        public boolean canUse() {
            return !Butterfly.this.isSleeping() && super.canUse();
        }
