...
public class Kiwi extends Animal {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new Kiwi.DoeStrippedGoal(1.0F,12,4));
    ...
    public Kiwi(EntityType<? extends Animal> p_27557_, Level p_27558_) {
    ...
    public class DoeStrippedGoal extends MoveToBlockGoal {
        ...
        public DoeStrippedGoal(double pSpeedModifier, int pSearchRange, int pVerticalSearchRange) {
            super(Kiwi.this, pSpeedModifier, pSearchRange, pVerticalSearchRange);
        }
        ...
        private void stripped(BlockState pState , BlockPos pos) {
            int j = 1 + Kiwi.this.level().random.nextInt(2);
            ItemStack itemstack = Kiwi.this.getItemBySlot(EquipmentSlot.MAINHAND);
            ...
            if(ran >=20) {
                if (itemstack.isEmpty()) {
                    Kiwi.this.setItemSlot(EquipmentSlot.MAINHAND, white.get(random.nextInt(white.size())).getDefaultInstance());
                    --j;
                }
                ...
                if (j > 0) {
                    Block.popResource(Kiwi.this.level(), this.blockPos, new ItemStack(ModBlocks.KIWI_SAPLING.get().asItem(), j));
                }
            }

            Kiwi.this.playSound(SoundEvents.SHOVEL_FLATTEN, 1.0F, 1.0F);
            Kiwi.this.level().setBlock(pos, Blocks.DIRT.defaultBlockState(),11);
        }

        public boolean canUse() {
            return !Kiwi.this.isSleeping() && super.canUse();
        }
