...
public class Cicada extends Animal implements GeoEntity{

    private AnimatableInstanceCache factory = new SingletonAnimatableInstanceCache(this);

    public static final float FLAP_DEGREES_PER_TICK = 74.48451F;
    public static final int TICKS_PER_FLAP = Mth.ceil(2.4166098F);
    private static final EntityDataAccessor<Byte> DATA_ID_FLAGS = SynchedEntityData.defineId(Cicada.class, EntityDataSerializers.BYTE);
    ...
    public SoundEvent getAmbientSound() {
        if (this.isResting()) {
            return this.random.nextInt(4) != 0 ? null : ModSounds.CICADA_AMBIENT.get();
    ...
    protected SoundEvent getHurtSound(DamageSource p_27451_) {
        return ModSounds.CICADA_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return ModSounds.CICADA_DEATH.get();
    }
    ...
    public static AttributeSupplier setAttributes() {
        return Animal.createAnimalAttributes()
                .add(Attributes.MAX_HEALTH, 5.0D).build();
    }
    ...
    protected void customServerAiStep(net.minecraft.server.level.ServerLevel pLevel) {
        ...
        if (this.isResting()) {
            ...
            if (this.level().getBlockState(blockpos1).isRedstoneConductor(this.level(), blockpos) ||
                    this.level().getBlockState(blockpos2).isRedstoneConductor(this.level(), blockpos)||
            ...
            if (this.random.nextInt(100) == 0 && this.level().getBlockState(blockpos1).isRedstoneConductor(this.level(), blockpos1)||
                    this.random.nextInt(100) == 0 && this.level().getBlockState(blockpos2).isRedstoneConductor(this.level(), blockpos2)||
                    this.random.nextInt(100) == 0 && this.level().getBlockState(blockpos3).isRedstoneConductor(this.level(), blockpos3)||
                    this.random.nextInt(100) == 0 && this.level().getBlockState(blockpos4).isRedstoneConductor(this.level(), blockpos4)) {
                this.setResting(true);
            }
        }
