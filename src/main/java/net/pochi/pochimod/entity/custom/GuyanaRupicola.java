...
public class GuyanaRupicola extends Animal implements FlyingAnimal {
    ...
    public GuyanaRupicola(EntityType<? extends Animal> entityType, Level level) {
    ...
    public class DoeStrippedGoal extends MoveToBlockGoal {
        ...
        public DoeStrippedGoal(double pSpeedModifier, int pSearchRange, int pVerticalSearchRange) {
            super(GuyanaRupicola.this, pSpeedModifier, pSearchRange, pVerticalSearchRange);
        }
        ...
        private void stripped(BlockState pState , BlockPos pos) {
            int j = 1 + GuyanaRupicola.this.level().random.nextInt(2);
            ItemStack itemstack = GuyanaRupicola.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty()) {
                GuyanaRupicola.this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(Items.APPLE));
                --j;
            }

            if (j > 0) {
                Block.popResource(GuyanaRupicola.this.level(), this.blockPos, new ItemStack(Items.APPLE, j));
            }

            GuyanaRupicola.this.playSound(SoundEvents.SHEEP_SHEAR, 1.0F, 1.0F);
            GuyanaRupicola.this.level().setBlock(pos, Blocks.AIR.defaultBlockState(),11);
        }

        public boolean canUse() {
            return !GuyanaRupicola.this.isSleeping() && super.canUse();
        }
