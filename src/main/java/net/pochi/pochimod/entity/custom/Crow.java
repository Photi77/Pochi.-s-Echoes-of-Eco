...
public class Crow extends Animal implements FlyingAnimal {
    ...
    public Crow(EntityType<? extends Animal> entityType, Level level) {
    ...
    public class DoeStrippedGoal extends MoveToBlockGoal {
        ...
        public DoeStrippedGoal(double pSpeedModifier, int pSearchRange, int pVerticalSearchRange) {
            super(Crow.this, pSpeedModifier, pSearchRange, pVerticalSearchRange);
        }
        ...
        protected void onReachedTarget() {
            if (net.neoforged.neoforge.common.NeoForge.EVENT_BUS.post(new net.neoforged.neoforge.event.entity.EntityMobGriefingEvent((net.minecraft.server.level.ServerLevel)Crow.this.level(), Crow.this)).canGrief()) {
                BlockState blockstate = Crow.this.level().getBlockState(this.blockPos);
        ...
        private void stripped(BlockState pState , BlockPos pos) {
            int j = 1 + Crow.this.level().random.nextInt(2);
            ItemStack itemstack = Crow.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty()) {
                Crow.this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(Items.BONE_MEAL));
                --j;
            }

            if (j > 0) {
                Block.popResource(Crow.this.level(), this.blockPos, new ItemStack(Items.BONE_MEAL, j*8));
            }

            Crow.this.playSound(SoundEvents.SHEEP_SHEAR, 1.0F, 1.0F);
            Crow.this.level().setBlock(pos, Blocks.AIR.defaultBlockState(),11);
        }

        public boolean canUse() {
            return !Crow.this.isSleeping() && super.canUse();
        }
