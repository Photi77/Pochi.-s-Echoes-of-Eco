...
public class HookArrowRenderer extends EntityRenderer<HookArrow> {
    ...
    private static void vertex(VertexConsumer pConsumer, Matrix4f p_254085_, PoseStack.Pose p_253962_, int pLightmapUV, float pX, int pY, int pU, int pV) {
        pConsumer.addVertex(p_254085_, pX - 0.5F, (float)pY - 0.5F, 0.0F).setColor(255, 255, 255, 255).setUv((float)pU, (float)pV).setOverlay(OverlayTexture.NO_OVERLAY).setLight(pLightmapUV).setNormal(p_253962_, 0.0F, 1.0F, 0.0F);
    }
    ...
    private static void stringVertex(float p_174119_, float p_174120_, float p_174121_, VertexConsumer pConsumer, PoseStack.Pose pPose, float p_174124_, float p_174125_) {
        ...
        f4 /= f6;
        f5 /= f6;
        pConsumer.addVertex(pPose.pose(), f, f1, f2).setColor(0, 0, 0, 255).setNormal(pPose, f3, f4, f5);
    }
}
