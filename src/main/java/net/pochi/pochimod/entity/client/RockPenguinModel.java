...
public class RockPenguinModel extends EntityModel<LivingEntityRenderState> {
    ...
    public void setupAnim(LivingEntityRenderState state) {
        ...
        if (state.walkAnimationSpeed > 0.01F) {
            ModAnimationDefinitions.ROCK_PENGUIN_WALK.bake(this.root()).applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 2f, 2.5f);
        } else if (false){
            this.etupirka.xRot = (float) Math.toRadians(85.0F);
            ModAnimationDefinitions.ROCK_PENGUIN_SWIM.bake(this.root()).applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 2f, 2.5f);
        }else {
