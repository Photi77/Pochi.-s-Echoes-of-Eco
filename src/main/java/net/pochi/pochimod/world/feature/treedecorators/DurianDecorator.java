...
public class DurianDecorator extends TreeDecorator {
    public static final MapCodec<DurianDecorator> CODEC =
            RecordCodecBuilder.mapCodec(instance ->
                    instance.group(
                            Codec.floatRange(0.0F, 1.0F)
                                    .fieldOf("probability")
                                    .forGetter(d -> d.probability)
                    ).apply(instance, DurianDecorator::new)
            );
    public final float probability;

    public DurianDecorator(float p_69976_) {
        this.probability = p_69976_;
    }

    protected TreeDecoratorType<?> type() {
        return ModTreeDecoratorType.DURIAN.get();
    }
                ...
                for(Direction direction : Direction.Plane.HORIZONTAL) {
                    if (randomsource.nextFloat() <= 0.25F) {
                        ...
                        if (pContext.isAir(blockpos)) {
                            pContext.setBlock(blockpos, ModBlocks.DURIAN_FRUIT.get().defaultBlockState().setValue(DurianBlock.AGE, Integer.valueOf(randomsource.nextInt(3))).setValue(DurianBlock.FACING, direction));
                        }
                    }
                }
