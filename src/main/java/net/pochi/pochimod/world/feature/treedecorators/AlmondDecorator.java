...
public class AlmondDecorator extends TreeDecorator {
    public static final MapCodec<AlmondDecorator> CODEC =
            RecordCodecBuilder.mapCodec(instance ->
                    instance.group(
                            Codec.floatRange(0.0F, 1.0F)
                                    .fieldOf("probability")
                                    .forGetter(d -> d.probability)
                    ).apply(instance, AlmondDecorator::new)
            );
    public final float probability;

    public AlmondDecorator(float p_69976_) {
        this.probability = p_69976_;
    }

    protected TreeDecoratorType<?> type() {
        return ModTreeDecoratorType.ALMOND.get();
    }
                ...
                for(Direction direction : Direction.Plane.HORIZONTAL) {
                    if (randomsource.nextFloat() <= 0.25F) {
                        ...
                        if (pContext.isAir(blockpos)) {
                            pContext.setBlock(blockpos, ModBlocks.ALMOND_FRUIT.get().defaultBlockState().setValue(AlmondBlock.AGE, Integer.valueOf(randomsource.nextInt(3))).setValue(AlmondBlock.FACING, direction));
                        }
                    }
                }
