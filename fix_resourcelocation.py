...
for filepath in java_files:
    ...
    # Fix: new ResourceLocation("namespace", "path") -> ResourceLocation.fromNamespaceAndPath("namespace", "path")
    # Pattern: new ResourceLocation( followed by quoted string, comma, quoted string )
    content = re.sub(
...
# Report remaining new ResourceLocation( usages
remaining_files = []
for filepath in java_files:
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
...
if remaining_files:
    print(f"\nRemaining new ResourceLocation( usages ({sum(c for _,c in remaining_files)} in {len(remaining_files)} files):")
